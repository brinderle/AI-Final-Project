...
def check_score(grid, players, food, score, goal_reached):
    ...
    if players[0] == players[1] or players[0] == players[2]:
        ...
        final = str(score)
        final = final + '\n'
        #f = open("25x25IslandReflexResults.txt", "a")
       # f.write(final)
        #f.close()
        quit()
...
# evaluate the score for one move instead of all moves
def reflex_eval_cur_state(moves, players, food, goal_reached):
    players_copy = copy.deepcopy(players)
    i = players_copy[0]
    players_copy[0] = i
    tmp_food = copy.deepcopy(food)
...
def minimax_eval(grid, players, food, walls, goal_reached):
    ...
    for i in moves:
        ...
        pred_players[0] = i
        tmp = get_min_score(grid, pred_players, food, walls, 1, goal_reached)
        vals.append(tmp)
    # vals is a list of tuples, where each tuple holds the score and goal_reached
    choices = []
    scores = []
    for i in vals:
        scores.append(i[0])
    for j in range(len(scores)):
        if scores[j] == max(scores):
            choices.append(scores[j])
    correspondingMoveIndex = scores.index(choices[random.randint(0, len(choices)-1)])
    return moves[correspondingMoveIndex], goal_reached
...
def get_max_score(grid, players, food, walls, depth, goal_reached):
    player_moves = get_moves(players[0], grid, walls)
    if depth == 5:
        score = reflex_eval_cur_state(player_moves, players, food, goal_reached)
    ...
    best_score = max(vals)
    goal_reached = goals_reached[vals.index(best_score)]
...
def get_min_score(grid, players, food, walls, depth, goal_reached):
    if depth == 5:
        moves = get_moves(players[0], grid, walls)
        score = reflex_eval_cur_state(moves, players, food, goal_reached)
        # print("score: " + str(score))
        return score, goal_reached
    ...
    best_score, goal_reached = get_max_score(grid, pred_players, food, walls, depth+1, goal_reached)
    
    return best_score, goal_reached

def moveMinimax(grid, players, food, walls, score, goal_reached):
    # player
    best_move, goal_reached = minimax_eval(grid, players, food, walls, goal_reached)
    players[0] = best_move
...
def Minimax_evalFunc(moves, players, food, grid, walls, goal_reached, depth):
    ...
    for i in moves:
        ...
        #print("closest_pos1:", closest_pos1)
        #print("closest_pos2:", closest_pos2)
        pred_players[0] = players[0]
        pred_players[1] = closest_pos1
        pred_players[2] = closest_pos2
    print("vals", vals)
    if depth < 1:
        print("vals depth",depth, vals)
        #time.sleep(2)
        return Minimax_evalFunc(moves, pred_players, food, grid, walls, goal_reached, depth+1)
    print("vals", vals)
    #time.sleep(3)
    return vals

def MinimaxMove(grid, players, food, walls, score, goal_reached):
    # player
    player_moves = get_moves(players[0], grid, walls)
    vals = Minimax_evalFunc(player_moves, players, food, grid, walls, goal_reached, 0)
    players[0] = choose_move(player_moves, vals)
...
def expectimove(grid, players, food, walls, score, goal_reached):
    player_moves = get_moves(players[0], grid, walls)
    vals = expectimax_evalFunc(player_moves, players, food, grid, walls, goal_reached, 0)
    players[0] = choose_move(player_moves, vals)
...
def expectimax_evalFunc(moves, players, food, grid, walls, goal_reached, depth):
    ...
    if depth < 1:
        print("vals depth",depth, vals)
        #time.sleep(2)
        return expectimax_evalFunc(moves, pred_players, food, grid, walls, goal_reached, depth+1)
    print("vals", vals)
    #time.sleep(3)
    return vals
