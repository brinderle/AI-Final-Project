...
def GenEnemy2Start(x, z):
    return '<Placement x="' + str(x + 0.5) + '" y="56" z="' + str(z + 0.5) + '" yaw="0"/>'

def GenGoalStart(x, z):
    return '<Placement x="' + str(x + 0.5) + '" y="56" z="' + str(z + 0.5) + '" yaw="0"/>'
...
def getXML(reset):
    ...
    xml = '''<?xml version="1.0" encoding="UTF-8" standalone="no" ?>
            ...
            # might want to add a new AgentSection for the new enemy agent, can be the same as the first enemy agent
    return xml
        ...
        if world_state.is_mission_running and world_state.number_of_observations_since_last_state > 0:
            ...
            if ob['Name'] == 'Player':
                ...
                if (current_pos[i] == (eCurr['x'], eCurr['z'])) or (current_pos[i] == (fCurr['x'], fCurr['z'])):
                    ...
                    break
                ##################################
                # added this section to stop if the player touches the goal agent
                ##################################
